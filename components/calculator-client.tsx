...
function insight(slug:string,v:Record<string,number>,r:{main:number;items:{label:string;value:string}[]}){const map:Record<string,string>={
 ...
 'pension':`물가상승률을 반영한 현재가치 기준 결과입니다. 실제 수령 조건과 세금은 상품별로 확인하세요.`,
...
export function calculate(slug:string,v:Record<string,number>){
 ...
 if(slug==='compound-interest'){const freq=v.frequency||12,periods=v.years*freq,pr=v.rate/100/freq;main=v.principal*Math.pow(1+pr,periods)+v.monthly*futureFactor(pr,periods)*(v.timing?1+pr:1);const inv=v.principal+v.monthly*periods;const real=main/Math.pow(1+(v.inflation||0)/100,v.years);label=`${v.years}년 후 예상 자산`;items=[{label:'총 투자원금',value:won(inv)},{label:'현재가치 환산',value:won(real)}]}
 ...
 if(slug==='dividend-calculator'){const gross=v.principal*v.yield/100;main=gross*(1-v.tax/100);const future=main*Math.pow(1+(v.growth||0)/100,v.years||0);label='연간 세후 배당금';items=[{label:'회당 예상 배당',value:won(main/(v.frequency||1))},{label:`${v.years}년 후 예상`,value:won(future)}]}
 ...
 if(slug==='fire-calculator'){const need=Math.max(0,v.expense-(v.otherIncome||0));main=need*12/(v.withdrawal/100);label='필요한 FIRE 목표 자산';let years=0,a=v.current;const realRate=(1+v.rate/100)/(1+(v.inflation||0)/100)-1;while(a<main&&years<100){a=a*(1+realRate)+v.monthly*12;years++}items=[{label:'현재 준비율',value:num(v.current/Math.max(main,1)*100)+'%'},{label:'예상 도달 나이',value:years>=100?'계산 범위 초과':num((v.age||0)+years)+'세'}]}
 ...
 if(slug==='loan'){const months=Math.max(1,n-(v.grace||0)),graceInterest=v.principal*m*(v.grace||0);let total=0;if(v.repayment===1){const principalPay=v.principal/months;main=principalPay+v.principal*m;total=v.principal+v.principal*m*(months+1)/2+graceInterest;label='첫 달 상환액'}else if(v.repayment===2){main=v.principal*m;total=v.principal+main*months+graceInterest;label='거치 후 월 이자'}else{main=m===0?v.principal/months:v.principal*m*Math.pow(1+m,months)/(Math.pow(1+m,months)-1);total=main*months+graceInterest;label='월 원리금 상환액'}items=[{label:'총 상환액',value:won(total)},{label:'총 이자',value:won(total-v.principal)}]}
 ...
 if(slug==='pension'){const realRate=(1+v.rate/100)/(1+(v.inflation||0)/100)-1,pm=realRate/12,months=v.years*12;const draw=pm===0?v.asset/months:v.asset*pm/(1-Math.pow(1+pm,-months));main=draw+(v.monthlyIncome||0);label='현재가치 기준 월 연금';items=[{label:'자산 인출분',value:won(draw)},{label:'연간 총수령액',value:won(main*12)}]}
...
export default function CalculatorClient({def}:{def:CalculatorDef}){
 ...
 return <><div className="calcLayout"><section className="calcPanel"><div className="panelTitle"><Calculator/><div><h2>값을 입력하세요</h2><p>조건을 변경하면 결과가 바로 계산됩니다.</p></div><button onClick={()=>setValues(defaults)}><RotateCcw/>초기화</button></div><div className="inputGrid">{def.fields.map(f=><label key={f.key}><span>{f.label}</span><div>{f.options?<select value={values[f.key]} onChange={e=>setValues({...values,[f.key]:Number(e.target.value)})}>{f.options.map(o=><option value={o.value} key={o.value}>{o.label}</option>)}</select>:<input type="number" value={values[f.key]} min={f.min??(f.default<0?-100:0)} step={f.step??1} onChange={e=>{const n=Number(e.target.value);setValues({...values,[f.key]:Number.isFinite(n)?n:0})}}/>}<b>{f.unit}</b></div>{f.unit==='원'&&<small className="amountHint">{koreanAmount(values[f.key])}</small>}</label>)}</div></section><section className="calcResult"><span className="resultLabel">{result.label}</span><strong>{result.display}</strong><div className="resultItems">{result.items.map(x=><div key={x.label}><span>{x.label}</span><b>{x.value}</b></div>)}</div>{scenarios.length>0&&<div className="scenarioMini"><div className="scenarioTitle"><span>수익률 시나리오 비교</span><small>기준 ±1%p</small></div>{scenarios.map(s=>{const max=Math.max(...scenarios.map(x=>Math.abs(x.value)),1);return <div className="scenarioRow" key={s.label}><span>{s.label}<small>{s.rate}%</small></span><i><b style={{width:`${Math.max(8,Math.abs(s.value)/max*100)}%`}}/></i><strong>{s.display}</strong></div>})}</div>}<div className="aiExplain"><TrendingUp/><div><b><Sparkles/> 결과 해석</b><p>{insight(def.slug,values,result)}</p></div></div><div className="resultActions"><button onClick={share}><Share2/>결과 공유</button><button className="save" onClick={save}>계산 저장 <ArrowRight/></button></div></section></div>{message&&<div className="calcToast">{message}</div>}</>
}
