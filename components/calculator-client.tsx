...
export default function CalculatorClient({def}:{def:CalculatorDef}){
 ...
 return <><div className="calcLayout"><section className="calcPanel"><div className="panelTitle"><Calculator/><div><h2>값을 입력하세요</h2><p>조건을 변경하면 결과가 바로 계산됩니다.</p></div><button onClick={()=>setValues(defaults)}><RotateCcw/>초기화</button></div><div className="inputGrid">{def.fields.map(f=><label key={f.key}><span>{f.label}</span><div><input type="number" value={values[f.key]} min={f.min??(f.default<0?-100:0)} step={f.step??1} onChange={e=>{const n=Number(e.target.value);setValues({...values,[f.key]:Number.isFinite(n)?n:0})}}/><b>{f.unit}</b></div></label>)}</div></section><section className="calcResult"><span className="resultLabel">{result.label}</span><strong>{result.display}</strong><div className="resultItems">{result.items.map(x=><div key={x.label}><span>{x.label}</span><b>{x.value}</b></div>)}</div>{scenarios.length>0&&<div className="scenarioMini"><div className="scenarioTitle"><span>수익률 시나리오 비교</span><small>기준 ±1%p</small></div>{scenarios.map(s=>{const max=Math.max(...scenarios.map(x=>Math.abs(x.value)),1);return <div className="scenarioRow" key={s.label}><span>{s.label}<small>{s.rate}%</small></span><i><b style={{width:`${Math.max(8,Math.abs(s.value)/max*100)}%`}}/></i><strong>{s.display}</strong></div>})}</div>}<div className="aiExplain"><Bot/><div><b><Sparkles/> 스마트 분석</b><p>입력한 조건을 기준으로 계산했습니다. 수익률과 기간을 바꿔 결과가 얼마나 달라지는지 비교해 보세요.</p></div></div><div className="resultActions"><button onClick={share}><Share2/>결과 공유</button><button className="save" onClick={save}>계산 저장 <ArrowRight/></button></div></section></div>{message&&<div className="calcToast">{message}</div>}</>
}
