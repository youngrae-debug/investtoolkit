...
금액을 시간으로 번역하는 개인 금융 내비게이션입니다. 목표 금액, 지금까지 모은 돈, 매달 모을 돈의 세 가지 질문으로 수익률 0% 원금 기준 목표 도착일을 먼저 보여줍니다. 결과 뒤에서만 수익률 가정, 조건 비교, 계획 저장과 이번 달 업데이트를 제공합니다.

## 제품 차별점

- 여러 금융 계산기의 모음이 아닌 하나의 목표 도착일 경험
- 첫 결과 전 세 가지 핵심 입력만 요구
- 금액 차이보다 빨라지거나 늦어지는 시간을 우선 표시
...
4. 수익률 0% 원금 기준 기간과 예상 도착 연월 확인
5. 선택적으로 수익률 가정과 조건 변경 비교
6. 계획을 현재 브라우저에 저장
7. 재방문 시 ‘이번 달 업데이트’로 지난 목표일과 비교

## 계산 엔진

계산 로직은 [`lib/simulation/engine.ts`](./lib/simulation/engine.ts)에 UI와 분리된 순수 함수로 구현되어 있습니다.
...
기본 계산은 저장하지 않습니다. 사용자가 ‘계획 저장’을 누른 경우에만 버전 1 스키마의 계획과 업데이트 기록을 localStorage에 보관합니다. 사용자는 데이터 백업, 백업 불러오기, 전체 삭제를 직접 실행할 수 있습니다.
