...
- 재방문 시 저장한 계획과 이번 달 돈 점검을 첫 화면에서 바로 연결
...
8. 계획을 현재 브라우저에 저장
9. 재방문 시 저축액 하나 또는 수입·총지출로 이번 달 돈을 점검하고 최근 3개월의 계획 대비 흐름과 다음 달 보완 금액을 확인
...
기본 계산은 저장하지 않습니다. 사용자가 ‘계획 저장’을 누른 경우에만 버전 7 스키마의 목표 날짜, 선택한 해결책, 월별 계획·실제 저축액, 선택적으로 입력한 월 수입·총지출과 변화 이유를 localStorage에 보관합니다. 이전 버전 데이터는 자동 이전합니다. 사용자는 데이터 백업, 백업 불러오기, 전체 삭제를 직접 실행할 수 있으며 저장 데이터가 없는 브라우저에서도 2MB 이하의 검증된 백업만 복원할 수 있습니다.
...
- [`docs/PRODUCT_ENGINEERING_LOOP.md`](./docs/PRODUCT_ENGINEERING_LOOP.md)
...
- 거래별 가계부, 세부 지출 분류, 계좌·카드 자동 연동 없음
